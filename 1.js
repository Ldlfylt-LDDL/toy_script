...
(function () {
    ...
    function injectToggleButton() {
      ...
      wrap.appendChild(b1); wrap.appendChild(b2); wrap.appendChild(b3); wrap.appendChild(b4); wrap.appendChild(b5);
    ...
    function injectStyles() {
      const css = `
        ...
        #scma-toggle-aero, #scma-toggle-mkt, #scma-toggle-chart, #scma-toggle-crecs, #scma-toggle-so {
        ...
        #scma-toggle-aero { border-radius: 8px 0 0 8px; border-left: 1px solid #3b82f6; }
        #scma-toggle-crecs { border-radius: 0; }
        #scma-toggle-so    { border-radius: 0 8px 8px 0; }
        #scma-toggle-aero:hover, #scma-toggle-mkt:hover, #scma-toggle-chart:hover, #scma-toggle-crecs:hover, #scma-toggle-so:hover { background: #2a4f7a; }
        /* ── SO analysis panel ── */
        #scma-so-panel {
    ...
    function initSimcoToolsLinks() {
      ...
      // Realm is the first purely-numeric path segment after /profitcalculator/
      // e.g. /profitcalculator/0/m → '0',  /profitcalculator/m → '0' (fallback)
      function getRealm() {
        const parts = location.pathname.split('/').filter(Boolean);
        for (const p of parts.slice(1)) { // skip 'profitcalculator'
      ...
      function scan() {
        // Sticky product-name headers (ST_ZH_TO_ID lookup acts as filter)
        document.querySelectorAll('div.sticky').forEach(el => {
          makeClickable(el);
        });
        // Material cost table: first <td> in each row
        document.querySelectorAll('tbody tr td:first-child').forEach(el => {
          makeClickable(el);
        });
      }
  
      // Debounced scan on DOM mutations (React SPA)
      let _stTimer = null;
      new MutationObserver(() => {
