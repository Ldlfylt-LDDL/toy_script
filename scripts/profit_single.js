// 全产品单步加工利润排名
// 配方: 本地 ALL_E 数据
// producedAnHour + wages: SimcoTools bulk /v1/realms/0/resources (paginated)
// VWAP: SimcoTools bulk /v1/realms/0/market/vwaps (quality=0)

const https = require('https');

// ── 节点和边（来自 chain_util.js）──
const ALL_N = [
...
const ALL_E = [
  ...
  // research (no inputs in edge list — raw production)
];

function fetchJson(url) {
  return new Promise((resolve, reject) => {
    https.get(url, { headers: { 'User-Agent': 'simco-analysis/1.0' } }, (res) => {
      let data = '';
...
async function main() {
  process.stdout.write('Fetching resources...\n');
  ...
  // Build lookup maps
  const resMap = {};  // id → {producedAnHour, wages}
  resources.forEach(r => { resMap[r.id] = { rate: r.producedAnHour || 0, wages: r.wages || 0 }; });

  const vwapMap = {};  // id → vwap (quality=0)
  (vwapData.vwaps || []).forEach(v => { if (v.quality === 0) vwapMap[v.resourceId] = v.vwap; });

  // Build recipe map from edges: id → [{sourceId, qty}]
  const recipeMap = {};
  ALL_E.forEach(e => {
  ...
  for (const node of ALL_N) {
    const { rate, wages } = resMap[node.id] || { rate: 0, wages: 0 };
    if (rate === 0) continue;  // no production data

    const selfVwap = vwapMap[node.id];
    if (!selfVwap) continue;  // no market price

    const recipe = recipeMap[node.id] || [];
    let matCostPerHr = 0;
    let missingVwap = false;
    for (const { sid, qty } of recipe) {
      const iv = vwapMap[sid];
      if (!iv) { missingVwap = true; break; }
      matCostPerHr += iv * qty * rate;
    }
    if (missingVwap) continue;

    const revenuePerHr = rate * selfVwap * (1 - FEE);
    const profitPerHr = revenuePerHr - matCostPerHr - wages;
    const grossMarginPct = (revenuePerHr - matCostPerHr) / revenuePerHr * 100;
    ...
    results.push({
      ...
      rate,
      wages,
      matCost: matCostPerHr,
      revenue: revenuePerHr,
      profit: profitPerHr,
  ...
  console.log('\n=== 全产品单步加工利润排名（按净利润/hr） ===\n');
  console.log('排名  产品                      产量/hr    原料成本/hr   工资/hr   收入/hr   净利润/hr  毛利%');
  console.log('-'.repeat(105));
  results.forEach((r, i) => {
    const rank = String(i + 1).padStart(3);
    const name = r.name.padEnd(26);
    const rate = r.rate.toFixed(1).padStart(9);
    const mat  = r.matCost.toFixed(0).padStart(12);
    const wg   = r.wages.toFixed(0).padStart(8);
    const rev  = r.revenue.toFixed(0).padStart(9);
    const pft  = r.profit.toFixed(0).padStart(10);
    const mg   = r.margin.toFixed(1).padStart(6);
    console.log(`${rank}  ${name}${rate}  ${mat}  ${wg}  ${rev}  ${pft}  ${mg}%`);
  });
  console.log(`\n合计: ${results.length} 个产品`);
}
